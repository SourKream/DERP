...
def get_arguments():
	parser = argparse.ArgumentParser(description='Ruber Derp with gold model')
...
def base_model(options):
	...
	gold = Input(shape=(options['MAX_RESP_LEN'],))
	...
	encoded_resp = shared_gru(resp_embed)
	encoded_gold = shared_gru(resp_embed)
	...
	attended_gold_ctxt, gold_alpha = attention_module( [encoded_ctxt, encoded_gold]  )
	if options['VANILLA']:
		merged_vec = keras.layers.concatenate([attended_gold_ctxt, attended_resp_ctxt, encoded_gold, encoded_resp], axis=-1)
	else:
		bilinear_ctxt_resp = BilinearLayer()([attended_resp_ctxt, encoded_resp])
		bilinear_gold_resp = BilinearLayer()([encoded_gold, encoded_resp])
		merged_vec = keras.layers.concatenate([attended_resp_ctxt, attended_gold_ctxt, encoded_resp, encoded_gold, bilinear_ctxt_resp, bilinear_gold_resp], axis=-1)
	dense_vec = Dense(options['DENSE_HIDDEN_STATE'], activation='tanh')(merged_vec)
	pred_vec = Dense(1, activation='sigmoid')(dense_vec)
	return Model([ctxt,gold, resp], pred_vec)
...
def create_model(options):	
	...
	pos_score = scoring_model([ctxt, gold_resp, pos_resp])
	neg_score = scoring_model([ctxt, gold_resp, neg_resp])
	score = SubtractLayer()([pos_score, neg_score]) # pos_score - neg_score
	model = Model(inputs=[ctxt, gold_resp, pos_resp, neg_resp], outputs= score)
