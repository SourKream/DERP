...
import h5py
def generate_X_Y(options,key='TRAIN', num_elem = -1, mode = 'RUBER'):
    X = []
    y = []
    assert mode in set(['RUBER', 'DERP'])
...
def data_generator(data_x, data_y, options):
    ...
    while True:
        for i in xrange(0, len(data_x), BATCH_SIZE):
            ...
            cur_batch_ctxt, cur_batch_gold_resp, cur_batch_pos_resp, cur_batch_neg_resp = [list(x) for x in zip(*cur_batch_x)]
            ...
            yield [cur_batch_ctxt_vec, cur_batch_gold_resp_vec, cur_batch_pos_resp_vec, cur_batch_neg_resp_vec], np.array(cur_batch_y) 
...
def convert_weights_from_hdf5(weights_hdf5_file):
    ...
    new_weights = []        
    new_weights.append(weights_hdf5['model_1']['embedding_1']['embeddings'][:])  #(50002, 300) 
    ...
    new_weights.append(weights_hdf5['model_1']['attention_layer_1']['ctxt_dense'][:]) #(1024, 512)
    new_weights.append(weights_hdf5['model_1']['attention_layer_1']['resp_dense'][:]) #(600, 512)
    new_weights.append(weights_hdf5['model_1']['attention_layer_1']['alpha_dense'][:]) #(512, 1)
    if 'bilinear_layer_1' in weights_hdf5['model_1']:
        new_weights.append(weights_hdf5['model_1']['bilinear_layer_1']['ctx_dense'][:]) # (1024, 600)
    if 'bilinear_layer_2' in weights_hdf5['model_1']:
        new_weights.append(weights_hdf5['model_1']['bilinear_layer_2']['ctx_dense'][:]) # (600,600)
    
    new_weights.append(weights_hdf5['model_1']['dense_1']['kernel'][:]) #VANILLA : (3248, 100) BI-MATCHING : (3250,100)
