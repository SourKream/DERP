...
def load_data(filename, num_dat_points=-1):
    with open(filename,'r') as f: 
        ...
        for line in csv.reader(f):
            dat.append(line)
            if num_dat_points!=-1:
                if len(dat) == num_dat_points:
                    break
          
    for i in range(len(dat)):
        dat[i] = [x.lower() for x in dat[i]] # !when making vocab all are lower case
    
    assert(all([len(x)==3 for x in dat]))
    dat_x = [x[0] + ' <EOU> ' + x[1] + ' <EOU>' for x in dat]
    dat_y = [x[2] + ' <EOU>' for x in dat]
    dat_z = ['<BEG> ' + x[2] for x in dat]

    return dat_x, dat_y, dat_z

def data_generator(data_x, data_y, data_z, vocab_dict):
    assert(0 not in inverse_map(vocab_dict))  # 0 is for masking 
    while True:
        for i in range(0, len(data_x), BATCH_SIZE):
            cur_batch_ctxt = data_x[i:i+BATCH_SIZE]
            cur_batch_gold_resp = data_y[i:i+BATCH_SIZE]
            cur_batch_gold_resp_helper = data_z[i:i+BATCH_SIZE]
 
            for i in range(len(cur_batch_x)):
                cur_batch_ctxt[i] = map_to_idx(my_tokenize(cur_batch_ctxt[i]), vocab_dict)
                cur_batch_gold_resp[i] = map_to_idx(my_tokenize(cur_batch_gold_resp[i]), vocab_dict)
                cur_batch_gold_resp_helper[i] = map_to_idx(my_tokenize(cur_batch_gold_resp_helper[i]), vocab_dict)
            ...
            cur_batch_gold_resp_helper_vec = pad_sequences(cur_batch_gold_resp_helper, maxlen=MAX_RESP_LEN, value=0, padding='post', truncating='post')
            yield [cur_batch_ctxt_vec, cur_batch_gold_resp_helper_vec], cur_batch_gold_resp_vec.flatten()
